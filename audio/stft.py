...
class STFT(torch.nn.Module):
    ...
    def transform(self, input_data): #[1, L]
        num_batches = input_data.size(0) # 1
        num_samples = input_data.size(1) # L

        self.num_samples = num_samples

        # similar to librosa, reflect-pad the input
        input_data = input_data.view(num_batches, 1, num_samples) #[1,1,212893]
        input_data = F.pad(
            input_data.unsqueeze(1),
            (int(self.filter_length / 2), int(self.filter_length / 2), 0, 0),
            mode='reflect')
        input_data = input_data.squeeze(1) #[1,1,213917]
        ...
        cutoff = int((self.filter_length / 2) + 1) # 513
        ...
        return magnitude, phase # [1,513,832]
...
class TacotronSTFT(torch.nn.Module):
    ...
    def mel_spectrogram(self, y):
        ...
        magnitudes = magnitudes.data # [1 513 832]
        mel_output = torch.matmul(self.mel_basis, magnitudes)
        mel_output = self.spectral_normalize(mel_output)
        return mel_output # [1 80 832]
